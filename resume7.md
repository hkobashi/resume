...
- `Argo-CD Image Updaterによるデプロイフローの汎用化`  
  ...
  サービスごとの個別対応を不要にした。
- `アプリリポジトリとマニフェストリポジトリの分離`  
  アプリコードとk8sマニフェストは更新タイミングが異なるため、責務を分離した。  
  アプリリポジトリはイメージビルド、マニフェストリポジトリはデプロイに専念する構成とし、  
  k8sへのデプロイ履歴を独立して追跡可能にした。  
  また、マニフェスト変更を別リポジトリに分離することでCIビルドの無限ループを構造的に防止した。  
  ※Argo CD公式ドキュメントのベストプラクティスに基づく設計。
